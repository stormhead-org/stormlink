...
import (
	"fmt"
	models "stormlink/server/graphql/models"
...
func ProtoToGraphQLUser(protoUser *authpb.User) (*models.UserResponse, error) {
    ...
    if protoUser.Avatar != nil {
        avatar = &models.UserAvatarResponse{ID: protoUser.Avatar.Id, URL: protoUser.Avatar.Url}
    }
    userInfo := make([]*models.UserInfoResponse, len(protoUser.UserInfo))
    for i, info := range protoUser.UserInfo {
        userInfo[i] = &models.UserInfoResponse{ID: info.Id, Key: info.Key, Value: info.Value}
    }
    ...
    for i, role := range protoUser.HostRoles {
        hostRoles[i] = &models.UserHostRoleResponse{
            ID: role.Id, Title: role.Title, Color: role.Color,
            CommunityRolesManagement: role.CommunityRolesManagement,
            HostUserBan: role.HostUserBan, HostUserMute: role.HostUserMute,
    ...
    for i, role := range protoUser.CommunitiesRoles {
        communityRoles[i] = &models.UserCommunityRoleResponse{
            ID: role.Id, Title: role.Title, Color: role.Color,
            CommunityRolesManagement: role.CommunityRolesManagement,
            CommunityUserBan: role.CommunityUserBan, CommunityUserMute: role.CommunityUserMute,
    ...
    return &models.UserResponse{
        ID: protoUser.Id, Name: protoUser.Name, Slug: protoUser.Slug,
        Avatar: avatar, Email: protoUser.Email, Description: protoUser.Description,
        UserInfo: userInfo, HostRoles: hostRoles, CommunitiesRoles: communityRoles,
        IsVerified: protoUser.IsVerified, CreatedAt: protoUser.CreatedAt, UpdatedAt: protoUser.UpdatedAt,
    }, nil
}
