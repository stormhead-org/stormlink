-- Миграция: добавление роли "@everyone" для существующих сообществ
...
WHERE NOT EXISTS (
    ...
    WHERE ru.user_id = c.owner_id AND ru.role_id = r.id
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM role_users ru 
    WHERE ru.user_id = cf.user_id AND ru.role_id = r.id
);
...
SELECT 
    ...
    COUNT(ru.user_id) as users_with_everyone_role
FROM communities c
LEFT JOIN roles r ON r.community_id = c.id
LEFT JOIN role_users ru ON ru.role_id = r.id AND r.title = '@everyone'
